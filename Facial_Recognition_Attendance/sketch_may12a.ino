    ...
    if (align_face(net_boxes, image_matrix, aligned_face) == ESP_OK){
        if (is_enrolling == 1){
            ...
            if (matched_id >= 0) {
                ...
                // Extract recognized name
                const char* recognized_name = NULL;
                if (matched_id < name_length)
                    recognized_name = recognize_face_matched_name[matched_id].c_str();
                else
                    recognized_name = "No Name";

                // Send attendance record
                sendAttendanceRecord(recognized_name, matched_id);

                // Update the image with the recognized name
                rgb_printf(image_matrix, FACE_COLOR_GREEN, "[%u] %s", matched_id, recognized_name);
